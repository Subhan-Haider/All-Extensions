...
(function () {
    window.addEventListener('DOMContentLoaded', () => {
        ...
        const GITHUB_CLIENT_IDS = [
            'Ov23lidOK9TfEr2LfV4l' // Your New OAuth App ID
        ];
        let activeClientId = GITHUB_CLIENT_IDS[0];
        ...
        if (ghLoginOAuth) {
            ghLoginOAuth.onclick = async () => {
                ...
                const tryLogin = async (clientId) => {
                    try {
                        const params = new URLSearchParams();
                        params.append('client_id', clientId);

                        // GitHub Apps (Iv...) don't use scope, only OAuth Apps (Ov...) do
                        if (clientId.startsWith('Ov')) {
                        ...
                        if (!res.ok) {
                            console.error('GitHub API Response Error:', { status: res.status, data: JSON.stringify(data) });
                            let errorDetail = data.error_description || data.error || data.message || `Status ${res.status}`;
                            if (typeof errorDetail === 'object') errorDetail = JSON.stringify(errorDetail);
                            throw new Error(`[HTTP ${res.status}] ${errorDetail}`);
                        }

                        activeClientId = clientId; // Success, save this ID for the poll
                ...
                for (let i = 0; i < GITHUB_CLIENT_IDS.length; i++) {
                    const clientId = GITHUB_CLIENT_IDS[i];
                    ghLoginOAuth.textContent = `🔄 Contacting GitHub (${i + 1}/${GITHUB_CLIENT_IDS.length})...`;
                    const error = await tryLogin(clientId);
                    ...
                    const label = i === 0 ? 'Primary' : i === 1 ? 'Fallback' : `Backup ${i - 1}`;
                    errors.push(`${label}: ${error.message}`);
                    console.warn(`${label} Client ID failed:`, error);
                }

                if (!loginSuccess) {
                    alert(`GitHub Auth Error: All Client IDs failed.\n\n${errors.join('\n')}\n\nPlease check your internet connection or verify the Client IDs in developer settings.`);
            ...
            ghUploadBtn.onclick = async () => {
                chrome.storage.local.get(['gh-token'], async (result) => {
                    ...
                    try {
                        ...
                        let uploaded = 0;
                        const CONCURRENCY = 4; // Reduced from 16 to 4 for better reliability with GitHub's Git backend
                        ...
                        const processFile = async (entry) => {
                            try {
                                ...
                                if (!isNewRepo) {
                                    try {
                                        const fileRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${entry.filename}`, {
                                            headers: { 'Authorization': `token ${token}` },
                                            cache: 'no-store' // CRITICAL: Prevent stale SHA from cache
                                        });
                                        if (fileRes.ok) {
                                ...
                                let putRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${entry.filename}`, {
                                    method: 'PUT',
                                    headers: {
                                        'Authorization': `token ${token}`,
                                        'Content-Type': 'application/json'
                                ...
                                // Handle 409 Conflict (SHA mismatch) with a single retry
                                if (putRes.status === 409) {
                                    const latestRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${entry.filename}`, {
                                        headers: { 'Authorization': `token ${token}` },
                                        cache: 'no-store'
                                    });
                                    if (latestRes.ok) {
                                        const latestData = await latestRes.json();
                                        putRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${entry.filename}`, {
                                            method: 'PUT',
                                            headers: {
                                                'Authorization': `token ${token}`,
                                                'Content-Type': 'application/json'
                                            ...
                                            body: JSON.stringify({
                                                message: `Turbo Sync (Retry): ${entry.filename} updated`,
                                                content: b64,
                                                sha: latestData.sha
                                            })
                                        });
...
function showAdvancedOpener() {
    ...
    function maybeToggleWebStore() {
        ...
        if (!extensionId && /^[a-z]{32}$/.test(val)) {
            extensionId = val;
        }
    ...
    function maybeSaveBack() {
        var isExtensionId = /^[a-z]{32}$/.test(getCwsOption('xid'));
...
function getFriendlyNameFromUrl(url) {
    ...
    if (/^[a-z]{32}$/.test(url)) return 'ID: ' + url;
...
function appendFileChooser() {
    ...
    progressDiv.insertAdjacentHTML('beforeend',
        '<br><br>' +
        'Visit Microsoft Edge Add-ons or other extension galleries<br>' +
