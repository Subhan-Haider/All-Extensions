...
class SearchEngineElement {
    ...
    _ondblclick_element(event) {
        let { target } = event;
        // In some cases, target might be null or exactly this.element (clicking background)
        if (!target || target === this.element) return;

        // Find the child element of this.element (the line element)
        while (target && target.parentNode !== this.element) {
            target = target.parentNode;
            // Stop if we reach the top of the document or somehow escape the element
